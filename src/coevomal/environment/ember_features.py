...
# ---------------------------------------------------------------------------
# VENDORED from the official EMBER project (elastic/ember, ember/features.py),
# MIT-licensed. The only modification is making `lief` an optional import (see
# below): we consume EMBER's already-extracted raw-feature JSON and use only
# the vectorization path, which is pure numpy + scikit-learn. Vendoring avoids
# the lief/ember install chain on Python 3.13 while keeping the *exact*
# canonical 2381-dim feature_version=2 vectorization for comparability.
# ---------------------------------------------------------------------------
import re
import hashlib
import numpy as np
...
# lief is ONLY required to extract raw features from PE *bytes*. This project
# consumes EMBER's already-extracted raw-feature JSON and only needs the
# vectorization path (process_raw_features), which is pure numpy + sklearn.
# So lief is made optional: absent it, PE extraction is disabled but
# vectorization works unchanged.
try:
    import lief
    LIEF_MAJOR, LIEF_MINOR, _ = lief.__version__.split('.')
except Exception:
    lief = None
    LIEF_MAJOR, LIEF_MINOR = '0', '9'
...
class SectionInfo(FeatureType):
    ...
    def process_raw_features(self, raw_obj):
        ...
        # NOTE: the original EMBER code passes a bare string here. Under the
        # legacy scikit-learn it shipped with, input_type="string" iterated the
        # string's characters; modern scikit-learn rejects a bare string. We
        # pass the explicit character list to reproduce that exact behavior.
...
class PEFeatureExtractor(object):
    ...
    def __init__(self, feature_version=2, print_feature_warning=True, features_file=''):
        ...
        if feature_version == 1:
            if lief is not None and not lief.__version__.startswith("0.8.3"):
            ...
            if lief is not None and not lief.__version__.startswith("0.9.0"):
                if print_feature_warning:
